...
public class CpvpUtilsMod implements ClientModInitializer {

    public static MinecraftClient mc = MinecraftClient.getInstance(); // Ensure mc is initialized correctly
    public static int hitCount; // Moved inside the class
    public static int breakingBlockTick; // Moved inside the class



    @Override
    public void onInitializeClient() {
        mc = MinecraftClient.getInstance(); // Reinitialize mc to ensure it's up-to-date
        new EnableOptimizerCommand().register(); // Register your optimizer command

        // Register the client tick event and call useOwnTicks directly
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            if (mc != null && mc.player != null) {
                InventorySafetyHandler.handleInventoryTick();  // Your inventory safety logic
                useOwnTicks();  // Handle your custom ticks directly here
            }
        });
    }
    ...
    public static void useOwnTicks() {
        if (mc == null || mc.player == null) {
            return; // Ensure Minecraft and player instances are valid
        }
    ...
    private static BlockState getBlockState(BlockPos pos) {
        ...
        return Blocks.AIR.getDefaultState(); // Default state if mc.world is null
    }
